...
js/        — scripts, split by feature (core/ → store.js → shared.js → features/ → bootstrap.js)
...
js/store.js                  — FlashcardDB class + the `db` singleton. Reads/writes localStorage, calls
                                core/fsrs.js's fsrs() when scoring a card, owns every derived-stats query.
js/shared.js                 — showToast(): floating notification, callable from any feature.
js/features/home.js          — Home tab: TEMPLATES.home + renderHome() + its helpers (getGreeting,
                                formatMinutes).
js/features/stats.js         — Stats tab: TEMPLATES.stats + renderStats().
js/features/learn.js         — Learn tab: TEMPLATES.learnList + renderLearnList() + openTopicStudy()
                                (builds the study queue and hands off into the Study feature via
                                core/state.js's `studySession`).
js/features/study.js         — Study session: TEMPLATES.studySession/finishScreen/noCardsScreen,
...
js/features/settings.js      — Settings tab: TEMPLATES.settings + renderSettings(), saveSetting(),
...
- `js/core/fsrs.js` functions are pure; keep storage/DOM concerns out of it and in `js/store.js` / the feature files instead.
- Script load order in `index.html` is load-bearing: `js/core/*` → `js/store.js` → `js/shared.js` → `js/features/*.js` → `js/bootstrap.js`. A file may use globals a prior file defined, never the reverse. When adding a new top-level `const`/`let`/`class`/`function`, check it isn't already declared elsewhere (`grep` across `js/`) — redeclaring the same top-level name in two loaded scripts throws a `SyntaxError`.
- When adding a new screen/feature: create `js/features/<name>.js` + `css/pages/<name>.css`, and add both a `<script>` and `<link>` tag to `index.html` in the right position (JS position matters for load order; CSS position doesn't). Only touch `js/core/`, `js/shared.js`, or `css/components.css` for something genuinely reused across ≥2 features — see the "verified by actual call sites" rule above.
- `js/core/` stays a folder (4 files: fsrs/state/navigation/theme); `js/store.js`, `js/shared.js`, and each `js/features/*.js` are flat single files, not one-file-per-folder — a folder should only exist here once it holds ≥2 files.
