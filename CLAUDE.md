...
There is no build/dev-server tooling in this repo. Since scripts are loaded via `<script src="...">` tags and topic data is fetched dynamically (`loadTopicsDynamic` in `src/main.js` injects `<script>` tags), opening `index.html` directly via `file://` will generally work, but serving over HTTP avoids any edge-case script-loading issues:
...
Create a new IIFE file under `src/data/` following the `finance.js` pattern (unique `topic.id`, unique `word.id` prefixes), then add its path to the `topicFiles` array in `loadTopicsDynamic()` in `src/main.js`.
